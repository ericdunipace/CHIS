...
fn_list <- verify_file_names_and_data_type(
  file_name_2023,
  file_name_2022,
  file_name_2021,
  stata_or_sas
...
chis <- chis_clean(chis_list) %>%
  ...
  left_join(
    y = aux_data$prism$census %>% select(-year),
    by = c("tract10", "survey_years", "survey_months")
  ) %>% # join prism census data
  left_join(
    y = aux_data$prism$county %>% select(-year),
    by = c("county", "survey_years", "survey_months")
  ) # join prism county data
...
if (length(missing_obs) > 0) {
  cn <- colnames(
    aux_data$prism$census %>% select(-starts_with("survey"), -tract10, -year)
  )
  cn_c <- gsub("tract10_", "county_", cn)

  for (j in seq_along(cn)) {
    if (is.null(chis[missing_obs, cn[j]])) {
      next
    }
...
civic_engagement <- c(
  ...
  "health_missed_days", # Number OF DAYS OF SCHOOL MISSED FOR HEALTH PROBLEM PAST MO;
  # there's also the fact that for 2022 and 2021, it was not asked if TA4 is "no"
  # so 2023 was coded the same in our analysis to make results comparable
...
fe.formula <- paste0(formula, "+ as.factor(county)")
...
attr(table_demographics, "type") <- list(
  ...
  "age_cat" = "categorical", # SELF-REPORTED AGE
  "srage" = "categorical", # SELF-REPORTED AGE
...
utils::write.csv(
  county_result_year,
  file = here::here("Outputs", "climiate_anxiety_map_data.csv"),
  row.names = FALSE
)
...
# get summary tables
fe.summ <- fe.model %>% summary()
print(fe.model)
fe.vcov <- vcov(fe.model)

# save basic summary outputs
utils::write.csv(
  fe.summ$coefficients,
  file = here::here("Outputs", "fe_model_summary.csv")
)
...
# utils::write.csv(beta_me, file = here::here("Outputs", "mixef_coef.csv"))
# utils::write.csv(
#   rep_beta_me,
